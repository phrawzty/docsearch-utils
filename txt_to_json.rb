...
OptionParser.new do |opts|
    ...
    opts.on('--outputdir DIR', 'Direcotry where .json will end up (same as sourcedir by default).') do |v|
        options[:outputdir] = v
    end
...
if File.writable?(options[:outputdir]) then
    puts 'Writing to ' + options[:outputdir]
else
    puts 'Cannot write to ' + options[:outputdir]
    exit 1
end
...
Dir.glob(options[:sourcedir] + '/*.txt') do |txtfile|
    ...
    File.open(options[:outputdir] + '/' + name + '.json', 'w') do |f|
